...
infisical run --env=prod --path="/strapi" -- npm run strapi export -- --no-encrypt --file ../"$file_name"-prod
aws s3 cp ../"$file_name".tar.gz s3://civiconnect-automated-strapi-backups/"$file_name"-prod.tar.gz
rm ../"$file_name"-prod.tar.gz